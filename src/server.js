...
// ── Middleware للتحقق من صلاحيات المدير (Admin Roles & Permissions) ──
const isAdmin = (req, res, next) => {
  // التحقق من أن المستخدم مسجل ولديه دور مدير 'admin' لمنع التلاعب
  if (req.user && req.user.role === 'admin') {
    next();
  } else {
    res.status(403).json({ error: 'صلاحيات مرفوضة: هذا الإجراء مخصص لمدير النظام فقط لتجنب التلاعب المالي' });
  }
};
...
// طلب رمز OTP للتسجيل أو استعادة كلمة المرور
app.post('/auth/request-otp', async (req, res) => {
  try {
    const { phone_number } = req.body;
    if (!phone_number) return res.status(400).json({ error: 'رقم الجوال مطلوب' });
    
    // توليد رمز افتراضي ثابت للبيئة الحالية
    const otp = '1234'; 
...
// إنشاء حساب عضو جديد والتحقق عبر الـ OTP
...
// إعادة تعيين كلمة المرور عبر الـ OTP (نسيت كلمة المرور)
